...
and accessible throughout the codebase via the `get_config()` function.
...
class AppConfig(BaseModel):
    ...
    @classmethod
    def parse_classifications(cls, v: Any) -> list[ClassificationConfig]:
        """Parse classifications from dict format in YAML."""
        if isinstance(v, dict):
            return [
                ClassificationConfig(column_name=name, registry_import=import_path) for name, import_path in v.items()
            ]
        return list(v)
    ...
    def get_classifications(self) -> dict[str, type[BaseRegistry[Any]]]:
        ...
        if self._registry_cache is None:
            object.__setattr__(
                self,
                "_registry_cache",
                {c.column_name: c.get_registry() for c in self.classifications},
            )
    ...
    def balance_sheet_labels(self) -> list[str]:
        return self.labels.balance_sheet
    ...
    def date_columns(self) -> list[str]:
        return self.labels.date_columns

    def label_columns(self) -> list[str]:
        """Get all label columns (balance sheet labels + date columns + classification keys)."""
        return self.labels.balance_sheet + self.labels.date_columns + list(self.get_classifications().keys())
